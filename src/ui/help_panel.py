...
功能说明：
    ...
    状态效果、伙伴系统、技能池六类信息。游戏内 F1 / 暂停菜单 / 主菜单均可打开。
    - 共 6 个标签页，始终可见（不随伙伴是否存在隐藏）
...
from src.entities.player import get_skill_pool
...
_PAGE_HEADER = {
    ...
    5: "休息房「强化」可学习，共 8 种（习得火球术后火苗术自动移除）",
}
...
class HelpPanel:
    ...
    # 标签页名称（顺序即显示顺序）
    TABS = ["操作指南", "元素反应", "房间类型", "状态效果", "伙伴系统", "技能"]

    def __init__(self):
        self.current_tab: int = 0  # 当前选中标签下标
        # 技能页数据源：模块级技能池副本（无玩家状态依赖，纯展示）
        self._skills = get_skill_pool()
        # 面板几何（居中）
    ...
    def _draw_current_page(self, screen, font, small_font) -> None:
        ...
        if self.current_tab == 0:
            ...
            # 技能页（5）：从技能池动态生成
            self._draw_skill_page(screen, font, small_font)
    ...
    def _draw_skill_page(self, screen, font, small_font) -> None:
        """技能页：技能池逐行展示（元素图标 + 名称 + 元信息 + 描述）。"""
        x, y = self._content_x, self._content_top + 8
        icon_size = 24
        row_h = 48
        for skill in self._skills:
            ...
            if skill.element is not Element.NONE:
                self._blit_element_icon(screen, skill.element, x, y, icon_size)
            # 名称（元素色；物理用金色）
            name_color = (
                ELEMENT_COLOR[skill.element]
                if skill.element is not Element.NONE else (255, 220, 120)
            )
            name_text = font.render(skill.name, True, name_color)
            screen.blit(name_text, (x + icon_size + 10, y))
            # 元信息：AP 消耗 / 射程 / 倍率
            meta_text = small_font.render(
                f"{skill.ap_cost} AP · {skill.range_cells} 格 · {skill.multiplier:g}×",
                True, (170, 170, 190),
            )
            screen.blit(meta_text, (x + icon_size + 170, y))
            # 描述
            desc_text = small_font.render(skill.desc, True, (215, 215, 225))
            screen.blit(desc_text, (x + icon_size + 10, y + 24))
            y += row_h
